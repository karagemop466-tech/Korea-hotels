...
for h in hotels:
    ...
    exception_ok = (not h.get('transitException')) or bool(h.get('transitPlan'))
    qualified.append(bool(h.get('fits') and h.get('officialUrl') and room and exception_ok))
exceptions = [h for h in hotels if h.get('transitException')]
...
print('Transit-exception records:', len(exceptions), '→', ', '.join(h['id'] for h in exceptions) or 'none')
